...
make_graph <- function(data) {
    ...
    plot(1:n, (data[1:n]), main=paste("step =", step), type="h", xlab="n", ylab="popularity")
...
make_plotly_graph <- function(test_data) {
	...
	p <- plot_ly(df, x=n, y=popularity, name="popularity", type="points") %>%
	add_trace(y=rep(mean(ud1), count_test), name="before", type="linear")
    cat(ud1, "\n")
	inc(ud2[length(ud2)], step)
    cat(ud2, "\n")
	p <- p %>%
    add_trace(y=rep(mean(ud2), count_test), name=paste("after adding", step), type="linear")
	return (p)
}
